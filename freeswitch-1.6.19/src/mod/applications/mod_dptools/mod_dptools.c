...
static int e_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (uuid && e_data) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[ TetRavX] : WHISPER CHANNEL UUID : %s\n",uuid);
	...
	if (zstr(data)) {
		...
		if (!strcasecmp((char *) data, "all")) {
			...
			while (switch_channel_ready(channel)) {
				for (x = 0; x < MAX_SPY; x++) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "[ TetRavX] : Free UUID LIST : %s\n",e_data.uuid_list[x]);
				...
				if (e_data.total) {
					for (x = 0; x < e_data.total && switch_channel_ready(channel); x++) {
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "[ TetRavX] : APPLICATION EXECUTING WITH UUID : %s\n",e_data.uuid_list[x]);
            ...
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[ TetRavX] : APPLICATION EAVESDROP WITH UUID : %s AND FLAG : %d\n",data, (int)flags);

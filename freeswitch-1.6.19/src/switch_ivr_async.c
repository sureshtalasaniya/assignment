		...
		while (switch_channel_up_nosig(tchannel) && switch_channel_ready(channel)) {
			...
			if (fcommand) {
				...
				for (d = fcommand; *d; d++) {
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Press DTMF is : %c\n", *d);
...
// 						switch_set_flag(ep, ED_MUX_READ);
// 						switch_clear_flag(ep, ED_MUX_WRITE);
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Process To Supervisor Only Speak and Can't Listen To Agent\n");
                        switch_set_flag(ep, ED_MUX_WRITE);
                        switch_clear_flag(ep, ED_MUX_READ);
...
// 						}
					...
					if (z) {
						if (ep->r_buffer) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.0\n");
						...
						if (ep->w_buffer) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.1\n");
						}
					}
				}
			}
...
//                 switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.2\n");
			...
			if (switch_buffer_inuse(ep->buffer) >= len) {
				switch_buffer_lock(ep->buffer);
//                 switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.3\n");
				while (switch_buffer_inuse(ep->buffer) >= len) {
					...
					if (changed || tchanged) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.4\n");
						if (changed) {
					...
					if (ep->tread_impl.number_of_channels != ep->read_impl.number_of_channels) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.5\n");
					...
					if ((status = switch_core_session_write_frame(session, &write_frame, SWITCH_IO_FLAG_NONE, 0)) != SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.6.0\n");
						break;
					}
// 					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "[TetRavX] : Netrix 1.6\n");
				}
				switch_buffer_unlock(ep->buffer);
			}
